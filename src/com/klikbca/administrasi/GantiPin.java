...
public class GantiPin {
	...
	public static void takeSnapShot(WebDriver webdriver) throws Exception {
		...
		// conver webdriver object to TakeScreenshot
		String path = prop.getProperty("GantiPin");// conver webdriver object to TakeScreenshot
		...
		st.path = path;
		st.outdir =  prop.getProperty("PDFGantiPin");
	}
	...
	@Test(priority = 6)
	public void inputpinbaru() throws Exception {
		Properties prop = new Properties();
		// prop.load(new FileInputStream("kbi.properties"));
		InputStream stream = null;
		stream = new FileInputStream("kbi.properties");
		prop.load(stream);
		driver.findElement(By.id("NewPIN")).sendKeys(prop.getProperty("pinbaru"));
		takeSnapShot(driver);

	}
	@Test(priority = 7)
	public void inputpinbaruConfirm() throws Exception {
		Properties prop = new Properties();
		// prop.load(new FileInputStream("kbi.properties"));
		InputStream stream = null;
		stream = new FileInputStream("kbi.properties");
		prop.load(stream);
		driver.findElement(By.id("PINConf")).sendKeys(prop.getProperty("pinbaru"));
		takeSnapShot(driver);
